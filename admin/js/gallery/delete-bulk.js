function deleteSelectedGallery() {
  const selected = Array.from(document.querySelectorAll('input[name="gallery-select[]"]:checked'))
  ...
  deleteModalMessage.textContent = `Are you absolutely sure you want to delete ${selected.length} selected item${selected.length > 1 ? 's' : ''}? This action cannot be undone.`;
  ...
  newButton.addEventListener('click', () => {
    ...
    selected.forEach(id => formData.append('gallery_ids[]', id));

    fetch(`${ROOT_URL}/admin/scripts/gallery/delete_bulk.php`, {
      method: 'POST',
      body: formData
    })
      .then(response => {
