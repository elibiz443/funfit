function deleteSelectedProjects() {
  const selected = Array.from(document.querySelectorAll('input[name="project-select[]"]:checked'))
  ...
  deleteModalMessage.textContent = `Are you absolutely sure you want to delete ${selected.length} selected project${selected.length > 1 ? 's' : ''}? This action cannot be undone.`;
  ...
  newButton.addEventListener('click', () => {
    ...
    fetch(`${ROOT_URL}/admin/scripts/projects/delete_bulk.php`, {
      method: 'POST',
      body: formData
    })
      .then(response => {
