<?php
  ...
  require __DIR__ . '/../../scripts/services/admin_index.php';
  require __DIR__ . '/../../scripts/services/create.php';
  require __DIR__ . '/../../scripts/services/search.php';
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Joan Wisehart - Services</title>
...
<body class="bg-gray-100 max-w-full overflow-x-hidden text-neutral-900">
  ...
  <main id="mainContent" class="min-h-screen w-[calc(100%-5rem)] ml-auto flex-1 flex flex-col overflow-hidden transition-all duration-900 ease-in-out" style="background-image: url('<?php echo ROOT_URL; ?>/images/admin/dashboard.webp');">
    ...
    <div class="bg-white relative z-20 w-[90%] mx-auto rounded-lg -mt-[10rem] lg:-mt-[13rem] mb-[6rem] shadow-lg shadow-neutral-800 border border-neutral-400">
      <div class="w-full py-3 px-10 bg-gradient-to-r from-indigo-900 to-purple-800 text-white font-semibold rounded-t-lg">
        <p>All Services</p>
      </div>

      <div class="flex space-x-4 absolute z-[90] right-12 top-[1.6rem] rounded-xl">
        <button id="openServiceModal" class="group relative cursor-pointer flex items-center justify-center bg-green-600 text-green-200 w-10 h-10 rounded-lg shadow-md shadow-green-800 hover:shadow-lg transition-all duration-700 ease-in-out">
          <div class="hidden absolute left-1/2 transform -translate-x-1/2 w-[6rem] -top-[2.6rem] py-1.5 bg-yellow-500 text-center text-sm text-yellow-900 rounded-md shadow-md shadow-yellow-900 group-hover:block transition-all duration-900 ease-in-out">
            <div class="bg-yellow-500 w-full relative z-10">Add Service</div>
        ...
        <button onclick="deleteSelectedServices()" class="group relative cursor-pointer flex items-center justify-center bg-red-600 text-red-200 w-10 h-10 rounded-lg shadow-md shadow-red-800 hover:shadow-lg transition-all duration-700 ease-in-out">
      ...
      <div class="px-6 pt-6 pb-10 grid grid-cols-1 md:grid-cols-2 gap-6">
        <?php if (!empty($services)): ?>
          <?php foreach ($services as $service): ?>
            <div class="bg-white p-6 rounded-xl shadow-lg shadow-neutral-400 hover:shadow-neutral-400/50 border border-neutral-300 relative group h-[15rem] transition-all duration-500 ease-in-out">
              <label class="cursor-pointer select-none opacity-0 group-hover:opacity-100 transition-all duration-500 ease-in-out">
                <input type="checkbox" id="service-select-<?= htmlspecialchars($service['id']) ?>" name="service-select[]" value="<?= htmlspecialchars($service['id']) ?>" class="hidden peer/selected">
              ...
              <div class="flex items-start mb-4 pr-32 mt-1">
                <h3 class="text-lg md:text-xl font-extrabold text-neutral-800"><?= htmlspecialchars($service['title']) ?></h3>
              </div>
              ...
              <div class="flex flex-col items-end absolute top-5 right-5 h-full">
                <div class="flex space-x-2 opacity-0 group-hover:opacity-100 transition-all duration-500 ease-in-out -mt-2 mb-2">
                  <button onclick="editService(<?= htmlspecialchars($service['id']) ?>)" class="cursor-pointer size-8 bg-indigo-100 text-indigo-700 rounded-full shadow-sm shadow-indigo-900 flex items-center justify-center hover:bg-indigo-200 transition-all duration-150 ease-in-out" title="Edit Service">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                      <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                      <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                    </svg>
                  </button>
                  <button onclick="openDeleteModal(<?= htmlspecialchars($service['id']) ?>, '<?php echo ROOT_URL; ?>/admin/scripts/services/delete.php')" class="cursor-pointer size-8 bg-red-100 text-red-600 rounded-full shadow-sm shadow-indigo-900 flex items-center justify-center hover:bg-red-200 transition-all duration-150 ease-in-out" title="Delete Service">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div class="flex-shrink-0">
                  <img src="<?= htmlspecialchars(ROOT_URL . ($service['thumbnail'] ?? '/images/admin/default-img.webp')) ?>" class="cursor-pointer relative z-10 size-20 object-cover shadow-xl border-4 border-white rounded-xl hover:scale-110 transition-all duration-500 ease-in-out" alt="Service Thumbnail" onclick="openImageModal('<?= htmlspecialchars(ROOT_URL . ($service['thumbnail'] ?? '/images/admin/default-img.webp')) ?>')">
                </div>

                <div class="mt-8">
                  <button onclick="viewService(<?= htmlspecialchars($service['id']) ?>)" class="flex items-center text-gray-800 text-xs cursor-pointer px-3 py-1.5 rounded-md hover:bg-gray-200 transition-all duration-500 ease-in-out">
                    View Service
          ...
          <?php endforeach; ?>
        <?php else: ?>
          <p class="text-white text-center bg-indigo-600 py-2 shadow-md shadow-gray-900 w-[60%] mx-auto">No service found.</p>
        <?php endif; ?>
      </div>
  ...
  <script src="<?php echo ROOT_URL; ?>/admin/js/service/add-modal.js"></script>
  <script src="<?php echo ROOT_URL; ?>/admin/js/service/edit-modal.js"></script>
  <script src="<?php echo ROOT_URL; ?>/admin/js/service/view-modal.js"></script>
  <script src="<?php echo ROOT_URL; ?>/admin/js/service/delete-modal.js"></script>
  <script src="<?php echo ROOT_URL; ?>/admin/js/service/delete-bulk.js"></script>
</body>
</html>
